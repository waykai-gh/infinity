...
// simple_nn.ts
// Запускается через: tsc simple_nn.ts && node simple_nn.js
    ...
    train(X: Matrix, y: Matrix, lr = 0.5, epochs = 5000): void {
      for (let e = 0; e < epochs; e++) {
        ...
        this.b2[0] = this.b2[0].map(
          (val, j) =>
            val - (lr * dZ2.reduce((s, r) => s + r[j], 0)) / X.length
        );
        ...
        this.b1[0] = this.b1[0].map(
          (val, j) =>
            val - (lr * dZ1.reduce((s, r) => s + r[j], 0)) / X.length
        );
  
        if (e % 1000 === 0) {
          const loss =
            error.reduce(
              (s, r) => s + r.reduce((ss, v) => ss + v * v, 0),
              0
            ) / X.length;
