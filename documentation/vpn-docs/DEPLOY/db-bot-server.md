...
1. Пользователь отправляет команду `/key` боту.
2. Бот проверяет/создаёт пользователя в БД.
3. Бот выбирает подходящий сервер (по плану).
4. VPN сервис генерирует VLESS ссылку.
...
- `sni` — Server Name Indication (для маскировки трафика, например `aikyn.kz` для free, `github.com` для premium)
...
bot.command('start', async (ctx) => {
  await ctx.reply('👋 Добро пожаловать! Используй /key для получения VPN ключа.');
});
...
### День 1: Архитектура и БД

1. **Спланировали структуру таблиц:**
   - User (пользователи с планами)
   - Server (VPN серверы)
   - VpnKey (выданные ключи)

2. **Создали таблицы в PostgreSQL:**
   - Через pgAdmin (GUI) или psql.
   - Добавили UNIQUE constraint на `telegramId`.

3. **Настроили соединение с БД:**
   - `DATABASE_URL` в `.env`
   - Инициализировали `pg.Pool`

### День 2: Генерация ключей

1. **Создали VpnService:**
   - Функция для выбора сервера по плану.
   - Генерация UUID и shortId.
   - Формирование VLESS URL.

2. **Тестировали вручную:**
   - Добавили тестового пользователя в БД.
   - Вызвали `VpnService.createKeyForUser()`.
   - Проверили результат в таблице `VpnKey`.

### День 3: Telegram Bot

1. **Добавили команду `/key` в бота.**
2. **Интегрировали `UserService`:**
   - Автоматическое создание пользователя при первом обращении.
   - Возврат существующего пользователя при повторном обращении.

3. **Запустили бота:**
   - `npm run dev`
   - Отправили `/key` из Telegram.
   - Получили VLESS ссылку.
